...
const SalesReports: React.FC = () => {
  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      if (reportType === 'project' && projectReport) {
        ...
        pdf.setFontSize(20)
        pdf.setFont('helvetica', 'bold')
        pdf.text('ConstructCorp Customer Report', margin, 20)
  ...
  const generatePDFReport = async () => {
    ...
    if (reportType === 'project' && projectReport) {
      reportContent = `
SALES REPORT - ${projectReport.project.name}
...
- Location: ${projectReport.project.location}
- Status: ${projectReport.project.status}
- Total Units: ${projectReport.total_units}
- Units Sold: ${projectReport.sold_units}
- Available Units: ${projectReport.available_units}
- Reserved Units: ${projectReport.reserved_units}
- Sales Rate: ${projectReport.sales_rate.toFixed(1)}%

FINANCIAL SUMMARY:
- Total Revenue: $${projectReport.total_revenue.toLocaleString()}
- Average Price: $${projectReport.average_price.toLocaleString()}
- Project Budget: $${projectReport.project.budget.toLocaleString()}
- Revenue vs Budget: ${((projectReport.total_revenue / projectReport.project.budget) * 100).toFixed(1)}%

MONTHLY SALES DATA:
${projectReport.monthly_sales.map(month => 
  `${month.month}: ${month.units_sold} units sold, $${month.revenue.toLocaleString()} revenue`
).join('\n')}

APARTMENT DETAILS:
${projectReport.apartments.map(apt => 
  `Unit ${apt.number} (Floor ${apt.floor}): ${apt.size_m2}m² - $${apt.price.toLocaleString()} - ${apt.status}${apt.buyer_name ? ` (${apt.buyer_name})` : ''}`
).join('\n')}
      `
    } else if (reportType === 'customer' && customerReport) {
      reportContent = `
...
- Buyers: ${customerReport.buyers}
- Interested Customers: ${customerReport.interested}
- Leads: ${customerReport.leads}
...
${customerReport.customers.map(customer => 
  `${customer.name} ${customer.surname} (${customer.status.toUpperCase()}): ${customer.email} | ${customer.phone || 'No phone'} | Added: ${format(new Date(customer.created_at), 'MMM dd, yyyy')}`
).join('\n')}
      `
    }
    ...
    a.href = url
    a.download = `${reportType === 'project' ? 'Sales' : 'Customer'}_Report_${format(new Date(), 'yyyy-MM-dd')}.txt`
