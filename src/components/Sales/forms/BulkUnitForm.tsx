...
import type { UnitType, BulkCreateParams } from '../types/salesTypes'

interface BulkUnitFormProps {
  visible: boolean
  unitType: UnitType
  onClose: () => void
  onSubmit: (params: BulkCreateParams) => void
}

const BulkUnitForm: React.FC<BulkUnitFormProps> = ({ visible, unitType, onClose, onSubmit }) => {
  const [formData, setFormData] = useState<BulkCreateParams>({
    floor_start: 1,
    floor_end: 10,
    units_per_floor: 4,
    base_size: 85,
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder={unitType === 'apartment' ? 'A' : unitType === 'garage' ? 'G' : 'R'}
              />
              <p className="text-xs text-gray-500 mt-1">
                Default: {unitType === 'apartment' ? 'A' : unitType === 'garage' ? 'G' : 'R'}
                (e.g., A101, G202)
              </p>
            </div>
            <div>
          ...
          <div className="bg-blue-50 p-4 rounded-lg mb-6">
            ...
            <p className="text-xs text-blue-600 mt-3">
              Units will be numbered: {formData.number_prefix || (unitType === 'apartment' ? 'A' : unitType === 'garage' ? 'G' : 'R')}
              {formData.floor_start}01, {formData.number_prefix || (unitType === 'apartment' ? 'A' : unitType === 'garage' ? 'G' : 'R')}
