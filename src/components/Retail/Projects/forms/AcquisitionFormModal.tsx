...
export const AcquisitionFormModal: React.FC<AcquisitionFormModalProps> = ({
  phase,
  onClose,
  onSuccess,
  contract
  ...
  const [formData, setFormData] = useState({
    ...
    land_area_m2: contract?.land_area_m2?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.supplier_id) {
        throw new Error('Morate odabrati dobavljača (prodavatelja zemljišta)')
      }

      if (!formData.land_area_m2) {
        throw new Error('Morate unijeti broj kvadrata')
      }
      ...
      const dataToSubmit = {
        ...
        land_area_m2: parseFloat(formData.land_area_m2),
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-gray-900">
              {contract ? 'Uredi stjecanje zemljišta' : 'Novo stjecanje zemljišta'}
            </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 px-4 py-3 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>Napomena:</strong> Unesite podatke o kupovini zemljišta. Dobavljač predstavlja prodavatelja zemljišta.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Dobavljač (Prodavatelj zemljišta) *
              </label>
              <select
                ...
                required
              >
                <option value="">Odaberi prodavatelja...</option>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Ako prodavatelj ne postoji u listi, dodajte ga prvo preko "Dobavljači" menija
              </p>
            </div>
            ...
            {formData.land_area_m2 && formData.contract_amount && (
              <div className="md:col-span-2">
                <div className="bg-green-50 border border-green-200 px-4 py-3 rounded-lg">
                  <p className="text-sm text-green-800">
                    <strong>Cijena po m²:</strong>{' '}
                    {(parseFloat(formData.contract_amount) / parseFloat(formData.land_area_m2)).toLocaleString('hr-HR', {
                      style: 'currency',
                      currency: 'EUR',
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="Dodatne informacije o stjecanju zemljišta..."
              />
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Spremam...' : contract ? 'Spremi promjene' : 'Kreiraj stjecanje'}
            </button>
          </div>
        </form>
      </div>
