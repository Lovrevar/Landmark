...
interface DocumentUploadModalProps {
  isOpen: boolean
  onClose: () => void
  onUploaded?: (document: DocumentWithRelations) => void
...
export const DocumentUploadModal: React.FC<DocumentUploadModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (!file) {
      ...
      errs.file = t('documents.upload_modal.errors.file_too_large', { limit: MAX_FILE_SIZE_MB })
    }
    ...
    if (!file || !categoryId) return // type-narrow
    ...
    try {
      ...
      toast.success(t('documents.upload_modal.toast.success'))
      onUploaded?.(result)
      onClose()
    } catch (err) {
      const msg =
  ...
  return (
    <Modal show={isOpen} onClose={handleClose} size="lg">
      ...
      <Modal.Body>
        <FilePickerField
          file={file}
          onChange={(f) => { setFile(f); clearError('file') }}
          error={errors.file}
          disabled={submitting}
        />
...
// ===========================================================================
// File picker — single file, drag-drop or click, any mime, 50MB cap (caller-validated)
// ===========================================================================
...
const FilePickerField: React.FC<FilePickerFieldProps> = ({ file, onChange, error, disabled }) => {
  const { t } = useTranslation()
  const toast = useToast()
  ...
  const handleSelect = (files: FileList | null) => {
    ...
    if (f.size > MAX_FILE_SIZE) {
      // Reject oversized files immediately rather than waiting for submit —
      // user gets feedback the moment they drop or pick a too-large file.
      toast.error(t('documents.upload_modal.errors.file_too_large', { limit: MAX_FILE_SIZE_MB }))
  ...
  return (
    ...
    >
      {!file ? (
        ...
        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 rounded-lg">
