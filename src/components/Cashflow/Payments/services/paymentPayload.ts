...
export function buildPaymentData(formData: PaymentFormData, createdBy: string | undefined) {
  ...
  // ('bank_account' | 'credit'). Mirrors invoiceService.handlePaymentSubmit so both
  // code paths write the same accounting_payments shape (including credit allocations).
  return {
