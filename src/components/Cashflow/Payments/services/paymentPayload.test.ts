...
describe('buildPaymentData', () => {
  ...
  describe('credit payment', () => {
    it('keeps the credit, nulls the bank account', () => {
  ...
  describe('cesija (third-party debt assignment)', () => {
    it('forces source to bank_account and carries the cesija fields, nulling own bank/credit', () => {
      const r = buildPaymentData({ ...base, is_cesija: true }, 'user-1')
      expect(r.is_cesija).toBe(true)
      expect(r.payment_source_type).toBe('bank_account') // forced, regardless of form value
      expect(r.company_bank_account_id).toBeNull()
      expect(r.credit_id).toBeNull()
    ...
    it('overrides a credit source: cesija still forces bank_account and nulls credit', () => {
