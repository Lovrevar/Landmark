      ...
      <div className="p-6">
        {weekLogs.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-600 mb-2">{t('dashboards.supervision.no_work_logs')}</p>
            <p className="text-sm text-gray-500">{t('dashboards.supervision.work_logs_appear')}</p>
          </div>
        ) : (
              ...
              >
                ...
                {log.blocker_details && (
                  <div className="bg-red-50 border border-red-200 p-3 rounded-lg mb-3">
                    <p className="text-xs font-medium text-red-800 mb-1">{t('dashboards.supervision.issue_details')}</p>
                    <p className="text-sm text-red-700">{log.blocker_details}</p>
                  </div>
                )}
                {log.notes && (
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs font-medium text-gray-700 mb-1">{t('dashboards.supervision.notes_label')}</p>
