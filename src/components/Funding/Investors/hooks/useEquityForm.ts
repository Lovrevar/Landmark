...
export function useEquityForm(onSaved: () => Promise<void>) {
  ...
  const addEquity = async () => {
    if (!newEquity.bank_id || !newEquity.amount || !newEquity.investment_date) {
      toast.warning(t('funding.equity_form.error_fields_required'))
      return
    }
    ...
    if (newEquity.payment_schedule === 'custom') {
      toast.warning(t('funding.equity_form.error_custom_schedule'))
      return
    }
    ...
    try {
      ...
      await onSaved()
      toast.success(t('funding.equity_form.add_success'))
    } catch (error) {
      console.error('Error adding equity:', error)
      toast.error(t('funding.equity_form.add_error'))
    }
  }
