...
const CreditExpenses: React.FC<CreditExpensesProps> = ({ creditId }) => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq('invoice_type', 'INCOMING_BANK_EXPENSES')
        .eq('bank_credit_id', creditId)
        .order('issue_date', { ascending: false })

      if (error) throw error

      const mapped: ExpenseInvoice[] = (data || []).map((inv: any) => {
  ...
  const handleToggle = () => {
    if (!expanded) fetchExpenses()
    setExpanded((v) => !v)
  }

  const totalExpenses = invoices.reduce((sum, inv) => sum + inv.total_amount, 0)
  const totalPaid = invoices.reduce((sum, inv) => sum + (inv.payment_amount || 0), 0)
    ...
    <div className="mt-6 pt-6 border-t border-gray-200">
        ...
        <div className="flex items-center space-x-3">
          {fetched && invoices.length > 0 && (
            <span className="text-sm font-semibold text-amber-700">
              Ukupno troškovi: €{totalExpenses.toLocaleString('hr-HR')}
            </span>
          )}
          {expanded ? (
      ...
      {expanded && (
        <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
          {loading ? (
            <div className="p-4">
              <LoadingSpinner message="Učitavanje troškova..." />
            </div>
          ) : invoices.length === 0 ? (
            <p className="text-sm text-gray-500 px-4 py-3">
              Nema evidentiranih troškova za ovaj kredit.
            </p>
          ) : (
