...
import { Send, Users, User, ArrowLeft, MessageCircle, Paperclip, X, FileText, Loader2, ChevronDown } from 'lucide-react'
...
const MessagePanel: React.FC<MessagePanelProps> = ({
  ...
  const memberUsers = conversation.participants.map(p => ({
    id: p.user_id,
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      <div className="relative flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        {onBack && (
          <button
            onClick={onBack}
        ...
        <div className="min-w-0 flex-1">
          ...
          {conversation.is_group && (
            ...
            >
              ...
              <ChevronDown
                className={`w-3 h-3 flex-shrink-0 transition-transform ${showMembers ? 'rotate-180' : ''}`}
              />
            </button>
          )}
        </div>
        {conversation.is_group && (
          <button
            type="button"
            onClick={() => setShowMembers(v => !v)}
            aria-label={t('chat.members', { count: conversation.participants.length })}
            className="flex-shrink-0 hidden sm:block"
          >
            <AvatarStack users={memberUsers} max={4} size="sm" />
          </button>
        )}
        {conversation.is_group && showMembers && (
          <GroupMembersPanel
