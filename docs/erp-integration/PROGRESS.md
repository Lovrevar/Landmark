...
- **Changes on `development` since the pause may invalidate the SQL.** In
  ...
  applying, or the migration will quietly revert their change. (Checked at the
  merge: only the baseline defines it.)
- **`development` has added a trigger the smoke test never ran against.**
  `20260910120000_contract_budget_realized_integrity.sql` makes
  `accounting_payments` the source of truth for `contracts.budget_realized` and
  adds `trg_sync_contract_budget_realized_from_invoice` (AFTER INSERT / DELETE /
  UPDATE OF `contract_id` on `accounting_invoices`). Promotion never writes
  `contract_id`, so phase 3 should be unaffected — but phase 4 re-links
  contracts after the re-import, and that is exactly what this trigger reacts
  to. It should help (re-linking now recomputes realization), but rehearse it.
