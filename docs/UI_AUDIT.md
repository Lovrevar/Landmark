...
2. [~] **Dates.** *(Screens done 2026-09-23: every English-month `format()` in the UI now goes through `formatDate` / `formatDateTime` / `formatMonthYear` in `utils/formatters.ts`, which render `05.01.2026.` in Croatian and `Jan 05, 2026` in English, and parse a date-only string locally so a `date` column stops showing the previous day. A Vitest guard fails the build if an English month name comes back outside the export generators. **Left:** the three PDF generators and the CSV headers — the exports batch, whose policy is recorded in docs/REPORTS.md — plus `DateInput`'s third format (`DD/MM/YYYY`) and the 59 native `type="date"` inputs.)* *(Re-measured 2026-09-21: 82 `format()` calls over 43 files use `MMM`/`MMMM`/`EEE` with **no** locale — English month and day names in the Croatian UI — and **no file anywhere imports a date-fns locale**. 59 native `type="date"` inputs in 35 files against 19 `DateInput` uses in 11 files. There is no shared date-display helper: `utils/dateOnly.ts` only parses, and two components keep private `formatDate` copies.)* 5+ formats overall.
3. [ ] **Status colours and labels.** The same status is coloured differently per screen: UNPAID is red, grey or yellow; On Hold is red, orange, grey or yellow; "Paid" is teal, orange, green, blue or red. Raw English DB enums show in ~45 places (measured 2026-09-21). Needs one shared status → {label, variant} map per domain. *(Done 2026-09-23 for the cross-module vocabularies: `utils/statusDisplay.ts` holds project, contract, retail contract, retail phase, unit, milestone, retail milestone and risk level, each with one label key and one colour, and a test that asserts every map covers exactly the values its CHECK allows. Invoice category joined `invoiceHelpers`. **Left:** the per-screen colour choices inside Cashflow that no shared map covers yet, and the ~16 Croatian literal maps in `invoiceHelpers`/`paymentHelpers`.)* *(Started: invoice payment status has one — `getInvoiceStatusVariant` / `getInvoiceStatusLabel` in `Cashflow/services/invoiceHelpers.ts` — used by Approvals, Supervision and Retail invoice/payment modals and Funding credit invoices. Still hand-rolled: Cashflow Invoices, Customers, InvoiceDetailView, OfficeSuppliers, SupplierDetailsModal, Cashflow Calendar, Retail Invoices, Supervision Invoices; and every non-invoice vocabulary.)*
...
5. [~] **Hardcoded strings.** *(Re-measured properly on 2026-09-23: **435** display strings — 225 English, 93 Croatian, 117 neutral — not the ~290/~89 recorded here. The **English** ones on screen are done: Sales and Retail, Reports, dashboards, Funding, Supervision, General, Cashflow, and the shell. Much of it was consolidation rather than translation — three orphaned namespaces already held complete Croatian for the Excel import modals and the bulk price update while the live components rendered English. **Left:** the ~93 Croatian literals that belong in the locale files, AiChat's 36 (Croatian-only by a documented decision), the ~420 literals in the export generators, and a handful of wording calls listed below.)*
