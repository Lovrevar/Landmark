/*
  # Add direction prefix filter support to invoice RPC functions

  Changes:
  - get_filtered_invoices: p_invoice_type now accepts 'INCOMING' or 'OUTGOING'
    in addition to 'ALL' and exact type values.
    'INCOMING' matches all INCOMING_* types; 'OUTGOING' matches all OUTGOING_* types.
  - get_invoice_statistics: same change applied to keep statistics in sync.

  This supports the new UI filter which separates direction (Ulazni/Izlazni)
  from category (Dobavljač/Ured/Investicije/Banka) filters.
*/
