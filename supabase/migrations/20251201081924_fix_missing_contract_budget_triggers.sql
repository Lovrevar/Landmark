/*
  # Fix Missing Contract Budget Triggers

  ## Problem
  
  The triggers that should update contracts.budget_realized when accounting_payments
  are added, updated, or deleted are missing from the database. This causes budget_realized
  to become stale when payments are deleted.

  ## Solution
  
  Recreate the triggers that were defined in previous migrations but are not present in the database.
  
  ## Changes
  
  1. Ensure the update_contract_budget_realized() function exists
  2. Recreate all three triggers (INSERT, UPDATE, DELETE) on accounting_payments
  3. Recalculate all existing contract budget_realized values to fix stale data
*/

-- Recreate the function (in case it was modified/removed)
...
-- Recalculate all existing contract budget_realized values to fix any stale data
UPDATE contracts c
SET budget_realized = (
  SELECT COALESCE(SUM(
    CASE 
